...
resource "aws_instance" "jenkins_agent_test" {
  ...
  ami                    = data.aws_ami.server_ami.id
  key_name               = "${var.key_name}"
  vpc_security_group_ids = [aws_security_group.jenkins_agent_sg_test.id]
  subnet_id              = var.subnet_id[0]
  ...
  tags = {
    Name        = "${var.environment}-jenkins_agent_test"
  }
...
connection {
  ...
  provisioner "local-exec" {
    command = "ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -u ec2-user -i '${self.public_ip},' --private-key ~/.ssh/mtckey -e 'pub_key=~/.ssh/mtckey.pub' ${path.root}/modules/ansible/playbooks/jenkins-agent.yaml"
  }
